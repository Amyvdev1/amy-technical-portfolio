...
const projectData = {
  "forgeflow-ai-automation": {
    eyebrow: "01 / Full-stack workflow automation",
    title: "ForgeFlow AI Automation",
    subtitle: "A reviewable result, not a black box.",
    description: "A personal full-stack code sample that makes validated inputs, local run history, optional Gemini execution, deterministic fallback behavior, and a human-review checkpoint visible.",
    stack: ["React", "TypeScript", "FastAPI", "Pydantic", "SQLite", "Docker", "GitHub Actions"],
    question: "How can a workflow use optional AI assistance while preserving a clear execution record, a predictable fallback, and a human decision point?",
  },
  relayops: {
...
function ForgeFlowStudy() {
  const [aiMode, setAiMode] = useState(false);
  const status = aiMode ? "DEGRADED / FALLBACK" : "COMPLETED / DETERMINISTIC";
  const response = aiMode
    ? `{
  "status": "degraded",
  ...
  return <div className="api-frame forgeflow-frame"><aside className="api-nav"><div className="mini-brand"><span>F</span>ForgeFlow</div><p>WORKFLOW CONTROL</p>{["Workflow input", "Execution mode", "Run history", "Human review"].map((item, index) => <a key={item} className={index === 1 ? "api-active" : ""}>{item}</a>)}<div className="api-version">v0.2 <span>Personal code sample</span></div></aside><section className="api-docs"><p className="mini-eyebrow">RUN / REVIEWABLE</p><h3>Make the execution path visible.</h3><p>A React/TypeScript interface sends a structured request to FastAPI. The backend validates it, persists a local run record, and surfaces whether an optional AI path ran or a deterministic fallback was used.</p><div className="endpoint"><span className="method">POST</span><code>/api/workflows/:id/runs</code></div><div className="api-tabs"><button onClick={() => setAiMode(false)} className={!aiMode ? "tab-active" : ""}>Deterministic run</button><button onClick={() => setAiMode(true)} className={aiMode ? "tab-active" : ""}>Optional AI fallback</button></div><div className="code-window"><div className="code-bar"><span /><span /><span /><b>{status}</b></div><pre>{response}</pre></div></section><aside className="api-rail"><div className="rail-card"><Code2 size={18} /><h4>Typed boundary</h4><p>Pydantic validates the request before a run record is created.</p></div><div className="rail-card"><ShieldCheck size={18} /><h4>Honest fallback</h4><p>Unavailable optional AI is recorded as degraded, rather than shown as silent success.</p></div><div className="rail-card"><FileCheck2 size={18} /><h4>Scope stated</h4><p>Personal sample with local SQLite data, Docker, focused tests, and CI; not a production service.</p></div></aside></div>;
}
